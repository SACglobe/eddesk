...
const Header: React.FC<HeaderProps> = ({ data }) => {
    ...
    return (
        <header className="bg-white border-b border-slate-200 w-full border-t-4 border-t-emerald-900 relative">
                ...
                <div className="flex justify-between items-center py-4">
                    <Link href="/" className="flex items-center gap-3 md:gap-4 group">
                        ...
                        <div className="flex flex-col max-w-[280px] md:max-w-md">
                            <span className={`text-lg md:text-xl font-bold text-slate-900 tracking-tighter uppercase group-hover:text-emerald-900 transition-colors leading-[1.1] ${styles.serif}`}>
                                {data.meta.schoolName}
                            </span>
                            {data.meta.motto && (
                                <span className="text-[9px] text-slate-400 uppercase tracking-[0.2em] leading-none mt-1 font-medium">
                                    {data.meta.motto}
                                </span>
                            )}
                        </div>
                    ...
                    <nav className="hidden lg:flex items-center space-x-8">
                        ...
                        >
                            ...
                            {isMoreOpen && (
                                <div className={`absolute top-full right-0 pt-2 w-56 z-[60] ${styles.animateFadeIn}`}>
            ...
            {isMenuOpen && (
                <div className={`lg:hidden bg-white border-b border-slate-200 ${styles.animateSlideDown}`}>
